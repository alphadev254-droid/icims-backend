    ...
    if (pendingTx.type === 'package_subscription') {
      ...
      const payment = await prisma.payment.create({
        data: {
          ...
          paidAt: new Date(),
gatewayResponse: req.rawBody ? req.rawBody.toString() : JSON.stringify(req.body),          gatewayCharge: req.body.charge ? parseFloat(req.body.charge) : 0,
    ...
    if (pendingTx.type === 'event_ticket') {
      ...
      const transaction = await prisma.transaction.create({
        data: {
          ...
          systemFeeRate: metadata.systemFeeRate || 0,
          paidAt: new Date(),
gatewayResponse: req.rawBody ? req.rawBody.toString() : JSON.stringify(req.body),
    ...
    if (pendingTx.type === 'donation') {
      ...
      const transaction = await prisma.transaction.create({
        data: {
          ...
          systemFeeRate: metadata.systemFeeRate || 0,
          paidAt: new Date(),
gatewayResponse: req.rawBody ? req.rawBody.toString() : JSON.stringify(req.body),          isGuest: metadata.isGuest === true,
          guestName: metadata.isGuest ? metadata.guestName : null,
          guestEmail: metadata.isGuest ? metadata.guestEmail : null,
          guestPhone: metadata.isGuest ? metadata.guestPhone : null,
        }
