...
export async function reconcileAdminPendingTransaction(req: Request, res: Response): Promise<void> {
  const id = String(req.params.id || '');
  const result = await reconcilePendingTransactionById(id, { mode: 'manual' });
  if (result.status === 'skipped') {
    res.status(404).json({ success: false, message: result.message, data: result });
    return;
  }
  if (result.status === 'failed') {
    res.status(409).json({ success: false, message: result.message, data: result });
    return;
  }
